...
c4_mi_05 <- read_excel("b2_mi_05.xlsx")
c4_mi_10 <- read_excel("b2_mi_10.xlsx")
c4_mi_15 <- read_excel("b2_mi_15.xlsx")
c4_mi_20 <- read_excel("b2_mi_20.xlsx")
c4_mi_25 <- read_excel("b2_mi_25.xlsx")
c4_mi_30 <- read_excel("b2_mi_30.xlsx")

######
# **** A4 Maximum likelihood Imputation --------
######

# Function to impute missing values using maximum likelihood based on normal distribution
impute_with_mle <- function(x) {
  # Calculate MLE parameters for the last 14 days
  last_14_days <- tail(x, 14)
  mu <- mean(last_14_days, na.rm = TRUE)
  sigma <- sd(last_14_days, na.rm = TRUE)
  
  ifelse(is.na(x), rnorm(sum(is.na(x)), mean = mu, sd = sigma), x)
}

# **************** A4.1 - 5 % -------------------
# Sort the dataframe by "district", "commune", and "on_set_day"
c4_mi_05 <- c4_mi_05 %>%
  arrange(commune, on_set_day)

# Group by "district" and "commune", and apply maximum likelihood imputation for missing values
c4_mi_05 <- c4_mi_05 %>%
  group_by(commune) %>%
  mutate(case = impute_with_mle(case))

# Clear grouping
c4_mi_05 <- c4_mi_05 %>% ungroup()
...
######
# **** B2 - Merge the data -------------
c4_inci_merge <- rbind(c4_inci_raw, c4_inci_mi_05, c4_inci_mi_10, 
	c4_inci_mi_15, c4_inci_mi_20, c4_inci_mi_25, c4_inci_mi_30)

c4_inci_merge$type <- factor(c4_inci_merge$type, 
                        levels = c("raw", "5 per", "10 per", 
                        	"15 per", "20 per", "25 per", "30 per"))

######
# **** B5- Ve bieu do ----------
c4_inci_raw <- subset(c4_inci_raw, select = -type)
c4_inci_mi_05 <- subset(c4_inci_mi_05, select = -type)
c4_inci_mi_10 <- subset(c4_inci_mi_10, select = -type)
c4_inci_mi_15 <- subset(c4_inci_mi_15, select = -type)
c4_inci_mi_20 <- subset(c4_inci_mi_20, select = -type)
c4_inci_mi_25 <- subset(c4_inci_mi_25, select = -type)
c4_inci_mi_30 <- subset(c4_inci_mi_30, select = -type)


colnames(c4_inci_raw) <- c("on_set_day", "case_raw", "incidence_rate_raw")
colnames(c4_inci_mi_05) <- c("on_set_day", "case_05", "incidence_rate_05")
colnames(c4_inci_mi_10) <- c("on_set_day", "case_10", "incidence_rate_10")
colnames(c4_inci_mi_15) <- c("on_set_day", "case_15", "incidence_rate_15")
colnames(c4_inci_mi_20) <- c("on_set_day", "case_20", "incidence_rate_20")
colnames(c4_inci_mi_25) <- c("on_set_day", "case_25", "incidence_rate_25")
colnames(c4_inci_mi_30) <- c("on_set_day", "case_30", "incidence_rate_30")

c4_graph_inci_merge <- merge(c4_inci_raw, c4_inci_mi_05, by = "on_set_day", all.x = TRUE)
c4_graph_inci_merge <- merge(c4_graph_inci_merge, c4_inci_mi_10, by = "on_set_day", all.x = TRUE)
c4_graph_inci_merge <- merge(c4_graph_inci_merge, c4_inci_mi_15, by = "on_set_day", all.x = TRUE)
c4_graph_inci_merge <- merge(c4_graph_inci_merge, c4_inci_mi_20, by = "on_set_day", all.x = TRUE)
c4_graph_inci_merge <- merge(c4_graph_inci_merge, c4_inci_mi_25, by = "on_set_day", all.x = TRUE)
c4_graph_inci_merge <- merge(c4_graph_inci_merge, c4_inci_mi_30, by = "on_set_day", all.x = TRUE)

c4_graph_inci_merge$on_set_day <- as.Date(c4_graph_inci_merge$on_set_day )
...
# 30 Magenta: "magenta"
# ******** B5.1 - Raw vs 5% ---------
c4_graph_raw_05 <- ggplot(data = c4_graph_inci_merge, aes(x = on_set_day)) + 
...
#####
# ******** B5.2 - Raw vs 10% ---------
c4_graph_raw_10 <- ggplot(data = c4_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
#####
# ******** B5.3 - Raw vs 15% ---------
c4_graph_raw_15 <- ggplot(data = c4_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
#####
# ******** B5.4 - Raw vs 20% ---------
c4_graph_raw_20 <- ggplot(data = c4_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
#####
# ******** B5.5 - Raw vs 25% ---------
c4_graph_raw_25 <- ggplot(data = c4_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
#####
# ******** B5.6 - Raw vs 30% ---------
c4_graph_raw_30 <- ggplot(data = c4_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
c4_plot_list_1 <- list(c4_graph_raw_05, c4_graph_raw_10, c4_graph_raw_15)

c4_plot_list_2 <- list(c4_graph_raw_20, c4_graph_raw_25, c4_graph_raw_30)

# Arrange the plots 
c4_combined_plot_0515 <- grid.arrange(
  grobs = c4_plot_list_1,
  ncol = 1  # Adjust the number of columns here if needed
)

# Explore the plot
ggsave(
	filename = "c4_1_combined_plot_0515.PNG", 
  plot = c4_combined_plot_0515, 
  width = 9, height = 6, 
  units = "in", dpi = 600)

# Arrange the plots 
c4_combined_plot_2030 <- grid.arrange(
  grobs = c4_plot_list_2,
  ncol = 1  # Adjust the number of columns here if needed
)

# Explore the plot
ggsave(
	filename = "c4_2_combined_plot_2030.PNG", 
  plot = c4_combined_plot_2030, 
  width = 9, height = 6, 
  units = "in", dpi = 600)
...
c4_raw_sum_cycle <- c4_raw_sum_lag %>%
...
c4_raw_sum_ccc_indexes <- c4_raw_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
...
c4_raw_ccc_indexes <- c4_raw_sum_ccc_indexes %>%
...
c4_mi_05_sum_cycle <- c4_mi_05_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
c4_mi_05_sum_ccc_indexes <- c4_mi_05_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
c4_mi_05_ccc_indexes <- c4_mi_05_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
c4_mi_10_sum_cycle <- c4_mi_10_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
c4_mi_10_sum_ccc_indexes <- c4_mi_10_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
c4_mi_10_ccc_indexes <- c4_mi_10_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
c4_mi_15_sum_cycle <- c4_mi_15_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
c4_mi_15_sum_ccc_indexes <- c4_mi_15_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
c4_mi_15_ccc_indexes <- c4_mi_15_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
c4_mi_20_sum_cycle <- c4_mi_20_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
c4_mi_20_sum_ccc_indexes <- c4_mi_20_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
c4_mi_20_ccc_indexes <- c4_mi_20_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
c4_mi_25_sum_cycle <- c4_mi_25_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
c4_mi_25_sum_ccc_indexes <- c4_mi_25_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
c4_mi_25_ccc_indexes <- c4_mi_25_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
c4_mi_30_sum_cycle <- c4_mi_30_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
c4_mi_30_sum_ccc_indexes <- c4_mi_30_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
c4_mi_30_ccc_indexes <- c4_mi_30_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
# **** C8 - Merge data -------------
######
c4_merge <- merge(c4_raw_ccc_indexes, c4_mi_05_ccc_indexes, by = "commune", all.x = TRUE)
c4_merge <- merge(c4_merge, c4_mi_10_ccc_indexes, by = "commune", all.x = TRUE)
c4_merge <- merge(c4_merge, c4_mi_15_ccc_indexes, by = "commune", all.x = TRUE)
c4_merge <- merge(c4_merge, c4_mi_20_ccc_indexes, by = "commune", all.x = TRUE)
c4_merge <- merge(c4_merge, c4_mi_25_ccc_indexes, by = "commune", all.x = TRUE)
c4_merge <- merge(c4_merge, c4_mi_30_ccc_indexes, by = "commune", all.x = TRUE)

c4_merge[is.na(c4_merge)] <- 0
...
c4_mi_05_1 <- c4_mi_05_1[, -which(names(c4_mi_05_1) == "period")]
c4_mi_10_1 <- c4_mi_10_1[, -which(names(c4_mi_10_1) == "period")]
c4_mi_15_1 <- c4_mi_15_1[, -which(names(c4_mi_15_1) == "period")]
c4_mi_20_1 <- c4_mi_20_1[, -which(names(c4_mi_20_1) == "period")]
c4_mi_25_1 <- c4_mi_25_1[, -which(names(c4_mi_25_1) == "period")]
c4_mi_30_1 <- c4_mi_30_1[, -which(names(c4_mi_30_1) == "period")]

# Rename the colummns
colnames(b1_raw_1) <- c("on_set_day", "commune", "case_raw")
colnames(c4_mi_05_1) <- c("on_set_day", "commune", "case_mi_05", "note_mi_05")
colnames(c4_mi_10_1) <- c("on_set_day", "commune", "case_mi_10", "note_mi_10")
colnames(c4_mi_15_1) <- c("on_set_day", "commune", "case_mi_15", "note_mi_15")
colnames(c4_mi_20_1) <- c("on_set_day", "commune", "case_mi_20", "note_mi_20")
colnames(c4_mi_25_1) <- c("on_set_day", "commune", "case_mi_25", "note_mi_25")
colnames(c4_mi_30_1) <- c("on_set_day", "commune", "case_mi_30", "note_mi_30")

# Merge the data
c4_crude_merge <- merge(b1_raw_1, c4_mi_05_1, by = c("on_set_day", "commune"), all=TRUE)
c4_crude_merge <- merge(c4_crude_merge, c4_mi_10_1, by = c("on_set_day", "commune"), all=TRUE)
c4_crude_merge <- merge(c4_crude_merge, c4_mi_15_1, by = c("on_set_day", "commune"), all=TRUE)
c4_crude_merge <- merge(c4_crude_merge, c4_mi_20_1, by = c("on_set_day", "commune"), all=TRUE)
c4_crude_merge <- merge(c4_crude_merge, c4_mi_25_1, by = c("on_set_day", "commune"), all=TRUE)
c4_crude_merge <- merge(c4_crude_merge, c4_mi_30_1, by = c("on_set_day", "commune"), all=TRUE)

######
# **** E2 Calculate the change **** --------
colnames(c4_crude_merge)
# 5 per
c4_crude_merge_05 <- c4_crude_merge %>%
	filter(note_mi_05 == "miss") %>%
	mutate(change_05 = case_raw - case_mi_05) 

c4_crude_05 <- c4_crude_merge_05 %>%
  summarise(
...
# 10 per
c4_crude_merge_10 <- c4_crude_merge %>%
	filter(note_mi_10 == "miss") %>%
	mutate(change_10 = case_raw - case_mi_10) 

c4_crude_10 <- c4_crude_merge_10 %>%
  summarise(
...
# 15 per
c4_crude_merge_15 <- c4_crude_merge %>%
	filter(note_mi_15 == "miss") %>%
	mutate(change_15 = case_raw - case_mi_15) 

c4_crude_15 <- c4_crude_merge_15 %>%
  summarise(
...
# 20 per
c4_crude_merge_20 <- c4_crude_merge %>%
	filter(note_mi_20 == "miss") %>%
	mutate(change_20 = case_raw - case_mi_20)

c4_crude_20 <- c4_crude_merge_20 %>%
  summarise(
...
# 25 per
c4_crude_merge_25 <- c4_crude_merge %>%
	filter(note_mi_25 == "miss") %>%
	mutate(change_25 = case_raw - case_mi_25)

c4_crude_25 <- c4_crude_merge_25 %>%
  summarise(
...
# 30 per
c4_crude_merge_30 <- c4_crude_merge %>%
	filter(note_mi_30 == "miss") %>%
	mutate(change_30 = case_raw - case_mi_30)

c4_crude_30 <- c4_crude_merge_30 %>%
  summarise(
...
# Merge all
c4_crude_all <- rbind(c4_crude_05, c4_crude_10,
	c4_crude_15, c4_crude_20,
	c4_crude_25, c4_crude_30)

c4_crude_all$method <- "4-maximum"

library(writexl)

write_xlsx(c4_crude_all,"c4_crude_all.xlsx")

######
# **** D3 -Percentage change in incidence rate ----------------
# Replace missing value

c4_graph_inci_merge <- replace(c4_graph_inci_merge, is.na(c4_graph_inci_merge), 0)

colnames(c4_graph_inci_merge)

c4_merge_ch_inci_05 <- c4_graph_inci_merge %>%
...
c4_merge_ch_inci_10 <- c4_graph_inci_merge %>%
...
c4_merge_ch_inci_15 <- c4_graph_inci_merge %>%
...
c4_merge_ch_inci_20 <- c4_graph_inci_merge %>%
...
c4_merge_ch_inci_25 <- c4_graph_inci_merge %>%
...
c4_merge_ch_inci_30 <- c4_graph_inci_merge %>%
...
c4_merge_ch_inci_sum <- rbind(c4_merge_ch_inci_05, c4_merge_ch_inci_10,
	c4_merge_ch_inci_15, c4_merge_ch_inci_20,
	c4_merge_ch_inci_25, c4_merge_ch_inci_30)

c4_merge_ch_inci_sum$method <- "4-maximum"

library(writexl)

write_xlsx(c4_merge_ch_inci_sum,"c4_ch_inci.xlsx")