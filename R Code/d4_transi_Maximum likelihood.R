...
d4_mi_05 <- read_excel("b3_mi_05.xlsx")
d4_mi_10 <- read_excel("b3_mi_10.xlsx")
d4_mi_15 <- read_excel("b3_mi_15.xlsx")
d4_mi_20 <- read_excel("b3_mi_20.xlsx")
d4_mi_25 <- read_excel("b3_mi_25.xlsx")
d4_mi_30 <- read_excel("b3_mi_30.xlsx")
...
######
# **** B2 - Merge the data -------------
d4_inci_merge <- rbind(d4_inci_raw, d4_inci_mi_05, d4_inci_mi_10, 
	d4_inci_mi_15, d4_inci_mi_20, d4_inci_mi_25, d4_inci_mi_30)

d4_inci_merge$type <- factor(d4_inci_merge$type, 
                        levels = c("raw", "5 per", "10 per", 
                        	"15 per", "20 per", "25 per", "30 per"))

######
# **** B5- Ve bieu do ----------
d4_inci_raw <- subset(d4_inci_raw, select = -type)
d4_inci_mi_05 <- subset(d4_inci_mi_05, select = -type)
d4_inci_mi_10 <- subset(d4_inci_mi_10, select = -type)
d4_inci_mi_15 <- subset(d4_inci_mi_15, select = -type)
d4_inci_mi_20 <- subset(d4_inci_mi_20, select = -type)
d4_inci_mi_25 <- subset(d4_inci_mi_25, select = -type)
d4_inci_mi_30 <- subset(d4_inci_mi_30, select = -type)


colnames(d4_inci_raw) <- c("on_set_day", "case_raw", "incidence_rate_raw")
colnames(d4_inci_mi_05) <- c("on_set_day", "case_05", "incidence_rate_05")
colnames(d4_inci_mi_10) <- c("on_set_day", "case_10", "incidence_rate_10")
colnames(d4_inci_mi_15) <- c("on_set_day", "case_15", "incidence_rate_15")
colnames(d4_inci_mi_20) <- c("on_set_day", "case_20", "incidence_rate_20")
colnames(d4_inci_mi_25) <- c("on_set_day", "case_25", "incidence_rate_25")
colnames(d4_inci_mi_30) <- c("on_set_day", "case_30", "incidence_rate_30")

d4_graph_inci_merge <- merge(d4_inci_raw, d4_inci_mi_05, by = "on_set_day", all.x = TRUE)
d4_graph_inci_merge <- merge(d4_graph_inci_merge, d4_inci_mi_10, by = "on_set_day", all.x = TRUE)
d4_graph_inci_merge <- merge(d4_graph_inci_merge, d4_inci_mi_15, by = "on_set_day", all.x = TRUE)
d4_graph_inci_merge <- merge(d4_graph_inci_merge, d4_inci_mi_20, by = "on_set_day", all.x = TRUE)
d4_graph_inci_merge <- merge(d4_graph_inci_merge, d4_inci_mi_25, by = "on_set_day", all.x = TRUE)
d4_graph_inci_merge <- merge(d4_graph_inci_merge, d4_inci_mi_30, by = "on_set_day", all.x = TRUE)

d4_graph_inci_merge$on_set_day <- as.Date(d4_graph_inci_merge$on_set_day )
...
# 30 Magenta: "magenta"
# ******** B5.1 - Raw vs 5% ---------
d4_graph_raw_05 <- ggplot(data = d4_graph_inci_merge, aes(x = on_set_day)) + 
...
#####
# ******** B5.2 - Raw vs 10% ---------
d4_graph_raw_10 <- ggplot(data = d4_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
#####
# ******** B5.3 - Raw vs 15% ---------
d4_graph_raw_15 <- ggplot(data = d4_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
#####
# ******** B5.4 - Raw vs 20% ---------
d4_graph_raw_20 <- ggplot(data = d4_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
#####
# ******** B5.5 - Raw vs 25% ---------
d4_graph_raw_25 <- ggplot(data = d4_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
#####
# ******** B5.6 - Raw vs 30% ---------
d4_graph_raw_30 <- ggplot(data = d4_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
d4_plot_list_1 <- list(d4_graph_raw_05, d4_graph_raw_10, d4_graph_raw_15)

d4_plot_list_2 <- list(d4_graph_raw_20, d4_graph_raw_25, d4_graph_raw_30)

# Arrange the plots 
d4_combined_plot_0515 <- grid.arrange(
  grobs = d4_plot_list_1,
  ncol = 1  # Adjust the number of columns here if needed
)

# Explore the plot
ggsave(
	filename = "d4_1_combined_plot_0515.PNG", 
  plot = d4_combined_plot_0515, 
  width = 9, height = 6, 
  units = "in", dpi = 600)

# Arrange the plots 
d4_combined_plot_2030 <- grid.arrange(
  grobs = d4_plot_list_2,
  ncol = 1  # Adjust the number of columns here if needed
)

# Explore the plot
ggsave(
	filename = "d4_2_combined_plot_2030.PNG", 
  plot = d4_combined_plot_2030, 
  width = 9, height = 6, 
  units = "in", dpi = 600)
...
d4_raw_sum_cycle <- d4_raw_sum_lag %>%
...
d4_raw_sum_ccc_indexes <- d4_raw_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
...
d4_raw_ccc_indexes <- d4_raw_sum_ccc_indexes %>%
...
d4_mi_05_sum_cycle <- d4_mi_05_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
d4_mi_05_sum_ccc_indexes <- d4_mi_05_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
d4_mi_05_ccc_indexes <- d4_mi_05_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
d4_mi_10_sum_cycle <- d4_mi_10_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
d4_mi_10_sum_ccc_indexes <- d4_mi_10_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
d4_mi_10_ccc_indexes <- d4_mi_10_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
d4_mi_15_sum_cycle <- d4_mi_15_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
d4_mi_15_sum_ccc_indexes <- d4_mi_15_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
d4_mi_15_ccc_indexes <- d4_mi_15_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
d4_mi_20_sum_cycle <- d4_mi_20_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
d4_mi_20_sum_ccc_indexes <- d4_mi_20_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
d4_mi_20_ccc_indexes <- d4_mi_20_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
d4_mi_25_sum_cycle <- d4_mi_25_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
d4_mi_25_sum_ccc_indexes <- d4_mi_25_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
d4_mi_25_ccc_indexes <- d4_mi_25_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
d4_mi_30_sum_cycle <- d4_mi_30_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
d4_mi_30_sum_ccc_indexes <- d4_mi_30_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
d4_mi_30_ccc_indexes <- d4_mi_30_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
# **** C8 - Merge data -------------
######
d4_merge <- merge(d4_raw_ccc_indexes, d4_mi_05_ccc_indexes, by = "commune", all.x = TRUE)
d4_merge <- merge(d4_merge, d4_mi_10_ccc_indexes, by = "commune", all.x = TRUE)
d4_merge <- merge(d4_merge, d4_mi_15_ccc_indexes, by = "commune", all.x = TRUE)
d4_merge <- merge(d4_merge, d4_mi_20_ccc_indexes, by = "commune", all.x = TRUE)
d4_merge <- merge(d4_merge, d4_mi_25_ccc_indexes, by = "commune", all.x = TRUE)
d4_merge <- merge(d4_merge, d4_mi_30_ccc_indexes, by = "commune", all.x = TRUE)

d4_merge[is.na(d4_merge)] <- 0
...
d4_mi_05_1 <- d4_mi_05_1[, -which(names(d4_mi_05_1) == "period")]
d4_mi_10_1 <- d4_mi_10_1[, -which(names(d4_mi_10_1) == "period")]
d4_mi_15_1 <- d4_mi_15_1[, -which(names(d4_mi_15_1) == "period")]
d4_mi_20_1 <- d4_mi_20_1[, -which(names(d4_mi_20_1) == "period")]
d4_mi_25_1 <- d4_mi_25_1[, -which(names(d4_mi_25_1) == "period")]
d4_mi_30_1 <- d4_mi_30_1[, -which(names(d4_mi_30_1) == "period")]

# Rename the colummns
colnames(b1_raw_1) <- c("on_set_day", "commune", "case_raw")
colnames(d4_mi_05_1) <- c("on_set_day", "commune", "case_mi_05", "note_mi_05")
colnames(d4_mi_10_1) <- c("on_set_day", "commune", "case_mi_10", "note_mi_10")
colnames(d4_mi_15_1) <- c("on_set_day", "commune", "case_mi_15", "note_mi_15")
colnames(d4_mi_20_1) <- c("on_set_day", "commune", "case_mi_20", "note_mi_20")
colnames(d4_mi_25_1) <- c("on_set_day", "commune", "case_mi_25", "note_mi_25")
colnames(d4_mi_30_1) <- c("on_set_day", "commune", "case_mi_30", "note_mi_30")

# Merge the data
d4_crude_merge <- merge(b1_raw_1, d4_mi_05_1, by = c("on_set_day", "commune"), all=TRUE)
d4_crude_merge <- merge(d4_crude_merge, d4_mi_10_1, by = c("on_set_day", "commune"), all=TRUE)
d4_crude_merge <- merge(d4_crude_merge, d4_mi_15_1, by = c("on_set_day", "commune"), all=TRUE)
d4_crude_merge <- merge(d4_crude_merge, d4_mi_20_1, by = c("on_set_day", "commune"), all=TRUE)
d4_crude_merge <- merge(d4_crude_merge, d4_mi_25_1, by = c("on_set_day", "commune"), all=TRUE)
d4_crude_merge <- merge(d4_crude_merge, d4_mi_30_1, by = c("on_set_day", "commune"), all=TRUE)

######
# **** E2 Calculate the change **** --------
colnames(d4_crude_merge)
# 5 per
d4_crude_merge_05 <- d4_crude_merge %>%
	filter(note_mi_05 == "miss") %>%
	mutate(change_05 = case_raw - case_mi_05) 

d4_crude_05 <- d4_crude_merge_05 %>%
  summarise(
...
# 10 per
d4_crude_merge_10 <- d4_crude_merge %>%
	filter(note_mi_10 == "miss") %>%
	mutate(change_10 = case_raw - case_mi_10) 

d4_crude_10 <- d4_crude_merge_10 %>%
  summarise(
...
# 15 per
d4_crude_merge_15 <- d4_crude_merge %>%
	filter(note_mi_15 == "miss") %>%
	mutate(change_15 = case_raw - case_mi_15) 

d4_crude_15 <- d4_crude_merge_15 %>%
  summarise(
...
# 20 per
d4_crude_merge_20 <- d4_crude_merge %>%
	filter(note_mi_20 == "miss") %>%
	mutate(change_20 = case_raw - case_mi_20)

d4_crude_20 <- d4_crude_merge_20 %>%
  summarise(
...
# 25 per
d4_crude_merge_25 <- d4_crude_merge %>%
	filter(note_mi_25 == "miss") %>%
	mutate(change_25 = case_raw - case_mi_25)

d4_crude_25 <- d4_crude_merge_25 %>%
  summarise(
...
# 30 per
d4_crude_merge_30 <- d4_crude_merge %>%
	filter(note_mi_30 == "miss") %>%
	mutate(change_30 = case_raw - case_mi_30)

d4_crude_30 <- d4_crude_merge_30 %>%
  summarise(
...
# Merge all
d4_crude_all <- rbind(d4_crude_05, d4_crude_10,
	d4_crude_15, d4_crude_20,
	d4_crude_25, d4_crude_30)

d4_crude_all$method <- "4-maximum"

library(writexl)

write_xlsx(d4_crude_all,"d4_crude_all.xlsx")

######
# **** D3 -Percentage change in incidence rate ----------------
# Replace missing value

d4_graph_inci_merge <- replace(d4_graph_inci_merge, is.na(d4_graph_inci_merge), 0)

colnames(d4_graph_inci_merge)

d4_merge_ch_inci_05 <- d4_graph_inci_merge %>%
...
d4_merge_ch_inci_10 <- d4_graph_inci_merge %>%
...
d4_merge_ch_inci_15 <- d4_graph_inci_merge %>%
...
d4_merge_ch_inci_20 <- d4_graph_inci_merge %>%
...
d4_merge_ch_inci_25 <- d4_graph_inci_merge %>%
...
d4_merge_ch_inci_30 <- d4_graph_inci_merge %>%
...
d4_merge_ch_inci_sum <- rbind(d4_merge_ch_inci_05, d4_merge_ch_inci_10,
	d4_merge_ch_inci_15, d4_merge_ch_inci_20,
	d4_merge_ch_inci_25, d4_merge_ch_inci_30)

d4_merge_ch_inci_sum$method <- "4-maximum"

library(writexl)

write_xlsx(d4_merge_ch_inci_sum,"d4_ch_inci.xlsx")