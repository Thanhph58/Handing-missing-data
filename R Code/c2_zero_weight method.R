...
c2_mi_05 <- read_excel("b2_mi_05.xlsx")
c2_mi_10 <- read_excel("b2_mi_10.xlsx")
c2_mi_15 <- read_excel("b2_mi_15.xlsx")
c2_mi_20 <- read_excel("b2_mi_20.xlsx")
c2_mi_25 <- read_excel("b2_mi_25.xlsx")
c2_mi_30 <- read_excel("b2_mi_30.xlsx")
...
######
# **** B2 - Merge the data -------------
c2_inci_merge <- rbind(c2_inci_raw, c2_inci_mi_05, c2_inci_mi_10, 
	c2_inci_mi_15, c2_inci_mi_20, c2_inci_mi_25, c2_inci_mi_30)

c2_inci_merge$type <- factor(c2_inci_merge$type, 
                        levels = c("raw", "5 per", "10 per", 
                        	"15 per", "20 per", "25 per", "30 per"))

######
# **** B5- Ve bieu do ----------
c2_inci_raw <- subset(c2_inci_raw, select = -type)
c2_inci_mi_05 <- subset(c2_inci_mi_05, select = -type)
c2_inci_mi_10 <- subset(c2_inci_mi_10, select = -type)
c2_inci_mi_15 <- subset(c2_inci_mi_15, select = -type)
c2_inci_mi_20 <- subset(c2_inci_mi_20, select = -type)
c2_inci_mi_25 <- subset(c2_inci_mi_25, select = -type)
c2_inci_mi_30 <- subset(c2_inci_mi_30, select = -type)


colnames(c2_inci_raw) <- c("on_set_day", "case_raw", "incidence_rate_raw")
colnames(c2_inci_mi_05) <- c("on_set_day", "case_05", "incidence_rate_05")
colnames(c2_inci_mi_10) <- c("on_set_day", "case_10", "incidence_rate_10")
colnames(c2_inci_mi_15) <- c("on_set_day", "case_15", "incidence_rate_15")
colnames(c2_inci_mi_20) <- c("on_set_day", "case_20", "incidence_rate_20")
colnames(c2_inci_mi_25) <- c("on_set_day", "case_25", "incidence_rate_25")
colnames(c2_inci_mi_30) <- c("on_set_day", "case_30", "incidence_rate_30")

c2_graph_inci_merge <- merge(c2_inci_raw, c2_inci_mi_05, by = "on_set_day", all.x = TRUE)
c2_graph_inci_merge <- merge(c2_graph_inci_merge, c2_inci_mi_10, by = "on_set_day", all.x = TRUE)
c2_graph_inci_merge <- merge(c2_graph_inci_merge, c2_inci_mi_15, by = "on_set_day", all.x = TRUE)
c2_graph_inci_merge <- merge(c2_graph_inci_merge, c2_inci_mi_20, by = "on_set_day", all.x = TRUE)
c2_graph_inci_merge <- merge(c2_graph_inci_merge, c2_inci_mi_25, by = "on_set_day", all.x = TRUE)
c2_graph_inci_merge <- merge(c2_graph_inci_merge, c2_inci_mi_30, by = "on_set_day", all.x = TRUE)

c2_graph_inci_merge$on_set_day <- as.Date(c2_graph_inci_merge$on_set_day )
...
# 30 Magenta: "magenta"
# ******** B5.1 - Raw vs 5% ---------
c2_graph_raw_05 <- ggplot(data = c2_graph_inci_merge, aes(x = on_set_day)) + 
...
#####
# ******** B5.2 - Raw vs 10% ---------
c2_graph_raw_10 <- ggplot(data = c2_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
#####
# ******** B5.3 - Raw vs 15% ---------
c2_graph_raw_15 <- ggplot(data = c2_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
#####
# ******** B5.4 - Raw vs 20% ---------
c2_graph_raw_20 <- ggplot(data = c2_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
#####
# ******** B5.5 - Raw vs 25% ---------
c2_graph_raw_25 <- ggplot(data = c2_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
#####
# ******** B5.6 - Raw vs 30% ---------
c2_graph_raw_30 <- ggplot(data = c2_graph_inci_merge, aes(x = on_set_day)) + 
  geom_line(aes(y = incidence_rate_raw, color = "Raw Data"), size = 1) +
...
c2_plot_list_1 <- list(c2_graph_raw_05, c2_graph_raw_10, c2_graph_raw_15)

c2_plot_list_2 <- list(c2_graph_raw_20, c2_graph_raw_25, c2_graph_raw_30)

# Arrange the plots 
c2_combined_plot_0515 <- grid.arrange(
  grobs = c2_plot_list_1,
  ncol = 1  # Adjust the number of columns here if needed
)

# Explore the plot
ggsave(
	filename = "c2_1_combined_plot_0515.PNG", 
  plot = c2_combined_plot_0515, 
  width = 9, height = 6, 
  units = "in", dpi = 600)

# Arrange the plots 
c2_combined_plot_2030 <- grid.arrange(
  grobs = c2_plot_list_2,
  ncol = 1  # Adjust the number of columns here if needed
)

# Explore the plot
ggsave(
	filename = "c2_2_combined_plot_2030.PNG", 
  plot = c2_combined_plot_2030, 
  width = 9, height = 6, 
  units = "in", dpi = 600)
...
c2_raw_sum_cycle <- c2_raw_sum_lag %>%
...
c2_raw_sum_ccc_indexes <- c2_raw_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
...
c2_raw_ccc_indexes <- c2_raw_sum_ccc_indexes %>%
...
c2_mi_05_sum_cycle <- c2_mi_05_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
c2_mi_05_sum_ccc_indexes <- c2_mi_05_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
c2_mi_05_ccc_indexes <- c2_mi_05_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
c2_mi_10_sum_cycle <- c2_mi_10_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
c2_mi_10_sum_ccc_indexes <- c2_mi_10_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
c2_mi_10_ccc_indexes <- c2_mi_10_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
c2_mi_15_sum_cycle <- c2_mi_15_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
c2_mi_15_sum_ccc_indexes <- c2_mi_15_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
c2_mi_15_ccc_indexes <- c2_mi_15_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
c2_mi_20_sum_cycle <- c2_mi_20_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
c2_mi_20_sum_ccc_indexes <- c2_mi_20_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
c2_mi_20_ccc_indexes <- c2_mi_20_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
c2_mi_25_sum_cycle <- c2_mi_25_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
c2_mi_25_sum_ccc_indexes <- c2_mi_25_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
c2_mi_25_ccc_indexes <- c2_mi_25_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
c2_mi_30_sum_cycle <- c2_mi_30_sum_lag %>%
    # Xac dinh cac su kien
    arrange(commune) %>%
    mutate(event = case_when(
        # Dich
...
c2_mi_30_sum_ccc_indexes <- c2_mi_30_sum_cycle %>%
    # Tinh ccc count
    group_by(commune) %>%
    mutate(ccc_count = rleid(commune, start_time, end_time)) %>%
	# Tinh ccc time
...
c2_mi_30_ccc_indexes <- c2_mi_30_sum_ccc_indexes %>%
  # Calculate the maximum ccc_count for each commune
  group_by(commune) %>%
...
# **** C8 - Merge data -------------
######
c2_merge <- merge(c2_raw_ccc_indexes, c2_mi_05_ccc_indexes, by = "commune", all.x = TRUE)
c2_merge <- merge(c2_merge, c2_mi_10_ccc_indexes, by = "commune", all.x = TRUE)
c2_merge <- merge(c2_merge, c2_mi_15_ccc_indexes, by = "commune", all.x = TRUE)
c2_merge <- merge(c2_merge, c2_mi_20_ccc_indexes, by = "commune", all.x = TRUE)
c2_merge <- merge(c2_merge, c2_mi_25_ccc_indexes, by = "commune", all.x = TRUE)
c2_merge <- merge(c2_merge, c2_mi_30_ccc_indexes, by = "commune", all.x = TRUE)

c2_merge[is.na(c2_merge)] <- 0
...
c2_mi_05_1 <- c2_mi_05_1[, -which(names(c2_mi_05_1) == "period")]
c2_mi_10_1 <- c2_mi_10_1[, -which(names(c2_mi_10_1) == "period")]
c2_mi_15_1 <- c2_mi_15_1[, -which(names(c2_mi_15_1) == "period")]
c2_mi_20_1 <- c2_mi_20_1[, -which(names(c2_mi_20_1) == "period")]
c2_mi_25_1 <- c2_mi_25_1[, -which(names(c2_mi_25_1) == "period")]
c2_mi_30_1 <- c2_mi_30_1[, -which(names(c2_mi_30_1) == "period")]

# Rename the colummns
colnames(b1_raw_1) <- c("on_set_day", "commune", "case_raw")
colnames(c2_mi_05_1) <- c("on_set_day", "commune", "case_mi_05", "note_mi_05")
colnames(c2_mi_10_1) <- c("on_set_day", "commune", "case_mi_10", "note_mi_10")
colnames(c2_mi_15_1) <- c("on_set_day", "commune", "case_mi_15", "note_mi_15")
colnames(c2_mi_20_1) <- c("on_set_day", "commune", "case_mi_20", "note_mi_20")
colnames(c2_mi_25_1) <- c("on_set_day", "commune", "case_mi_25", "note_mi_25")
colnames(c2_mi_30_1) <- c("on_set_day", "commune", "case_mi_30", "note_mi_30")

# Merge the data
c2_crude_merge <- merge(b1_raw_1, c2_mi_05_1, by = c("on_set_day", "commune"), all=TRUE)
c2_crude_merge <- merge(c2_crude_merge, c2_mi_10_1, by = c("on_set_day", "commune"), all=TRUE)
c2_crude_merge <- merge(c2_crude_merge, c2_mi_15_1, by = c("on_set_day", "commune"), all=TRUE)
c2_crude_merge <- merge(c2_crude_merge, c2_mi_20_1, by = c("on_set_day", "commune"), all=TRUE)
c2_crude_merge <- merge(c2_crude_merge, c2_mi_25_1, by = c("on_set_day", "commune"), all=TRUE)
c2_crude_merge <- merge(c2_crude_merge, c2_mi_30_1, by = c("on_set_day", "commune"), all=TRUE)

######
# **** E2 Calculate the change **** --------
colnames(c2_crude_merge)
# 5 per
c2_crude_merge_05 <- c2_crude_merge %>%
	filter(note_mi_05 == "miss") %>%
	mutate(change_05 = case_raw - case_mi_05) 

c2_crude_05 <- c2_crude_merge_05 %>%
  summarise(
...
# 10 per
c2_crude_merge_10 <- c2_crude_merge %>%
	filter(note_mi_10 == "miss") %>%
	mutate(change_10 = case_raw - case_mi_10) 

c2_crude_10 <- c2_crude_merge_10 %>%
  summarise(
...
# 15 per
c2_crude_merge_15 <- c2_crude_merge %>%
	filter(note_mi_15 == "miss") %>%
	mutate(change_15 = case_raw - case_mi_15) 

c2_crude_15 <- c2_crude_merge_15 %>%
  summarise(
...
# 20 per
c2_crude_merge_20 <- c2_crude_merge %>%
	filter(note_mi_20 == "miss") %>%
	mutate(change_20 = case_raw - case_mi_20)

c2_crude_20 <- c2_crude_merge_20 %>%
  summarise(
...
# 25 per
c2_crude_merge_25 <- c2_crude_merge %>%
	filter(note_mi_25 == "miss") %>%
	mutate(change_25 = case_raw - case_mi_25)

c2_crude_25 <- c2_crude_merge_25 %>%
  summarise(
...
# 30 per
c2_crude_merge_30 <- c2_crude_merge %>%
	filter(note_mi_30 == "miss") %>%
	mutate(change_30 = case_raw - case_mi_30)

c2_crude_30 <- c2_crude_merge_30 %>%
  summarise(
...
# Merge all
c2_crude_all <- rbind(c2_crude_05, c2_crude_10,
	c2_crude_15, c2_crude_20,
	c2_crude_25, c2_crude_30)

c2_crude_all$method <- "2-weight"

library(writexl)

write_xlsx(c2_crude_all,"c2_crude_all.xlsx")

######
# **** D3 -Percentage change in incidence rate ----------------
# Replace missing value

c2_graph_inci_merge <- replace(c2_graph_inci_merge, is.na(c2_graph_inci_merge), 0)

colnames(c2_graph_inci_merge)

c2_merge_ch_inci_05 <- c2_graph_inci_merge %>%
...
c2_merge_ch_inci_10 <- c2_graph_inci_merge %>%
...
c2_merge_ch_inci_15 <- c2_graph_inci_merge %>%
...
c2_merge_ch_inci_20 <- c2_graph_inci_merge %>%
...
c2_merge_ch_inci_25 <- c2_graph_inci_merge %>%
...
c2_merge_ch_inci_30 <- c2_graph_inci_merge %>%
...
c2_merge_ch_inci_sum <- rbind(c2_merge_ch_inci_05, c2_merge_ch_inci_10,
	c2_merge_ch_inci_15, c2_merge_ch_inci_20,
	c2_merge_ch_inci_25, c2_merge_ch_inci_30)

c2_merge_ch_inci_sum$method <- "2-weight"

library(writexl)

write_xlsx(c2_merge_ch_inci_sum,"c2_ch_inci.xlsx")
